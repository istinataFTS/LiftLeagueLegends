  ...
  group('SecureStorageVoiceCredentialService', () {
    ...
    group('onPicovoiceKeyChanged', () {
      test('emits an event when the key is set', () async {
        when(() => storage.write(key: _kPicovoiceKey, value: 'new-key'))
            .thenAnswer((_) async {});
      ...
      test('emits one event per set call', () async {
        when(() => storage.write(key: _kPicovoiceKey, value: any(named: 'value')))
            .thenAnswer((_) async {});

        final events = <void>[];
        final sub = service.onPicovoiceKeyChanged.listen((_) => events.add(null));
      ...
      test('getPicovoiceAccessKey does NOT emit an event (read-only)', () async {
        when(() => storage.read(key: _kPicovoiceKey))
            .thenAnswer((_) async => 'some-key');
      ...
      test('stream delivers to multiple concurrent listeners', () async {
        when(() => storage.write(key: _kPicovoiceKey, value: 'k'))
            .thenAnswer((_) async {});

...
void main() {
  ...
  group('BlocFactoryRegistrationRule', () {
    test(
      'passes when all BLoCs are registerFactory (explicit generic)',
      () async {
    ...
    test(
      'passes when all BLoCs are registerFactory (inferred generic)',
      () async {
        final repo = FakeRepoView({
          'lib/injection/modules/register_log_module.dart':
    ...
    test(
      'reports a violation for registerLazySingleton<XxxBloc> (explicit generic)',
      () async {
        final repo = FakeRepoView({
    ...
    test(
      'reports a violation for registerLazySingleton<XxxCubit> (explicit generic)',
      () async {
        final repo = FakeRepoView({
          'lib/injection/modules/register_foo_module.dart':
    ...
    test(
      'waiver with reason shorter than 10 chars does not suppress violation',
      () async {
        final repo = FakeRepoView({
          'lib/injection/modules/register_foo_module.dart':
    ...
    test(
      'passes for multi-line inferred-generic singleton with waiver on preceding line',
      () async {
        final repo = FakeRepoView({
          'lib/injection/modules/register_foo_module.dart':
    ...
    test(
      'reports a violation with correct line number for multi-line inferred-generic singleton with no waiver',
      () async {
        final repo = FakeRepoView({
          'lib/injection/modules/register_foo_module.dart':
              'sl.registerLazySingleton(\n'
    ...
    test(
      'reports a violation when waiver is on the lambda line of the multi-line block, not the preceding line',
      () async {
